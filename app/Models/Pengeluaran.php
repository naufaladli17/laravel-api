...
class Pengeluaran extends Model
{
    ...
    public function getRows()
    {
        //API
        $token = $this->getToken();
        $urlApi = 'https://training-bios2.kemenkeu.go.id/api/get/data/keuangan/akuntansi/pengeluaran';
        ...
        if ($response->successful()) {
            $check_data_pengeluaran = $response->json();
        }

        $check_data_pengeluaran = Arr::map($check_data_pengeluaran['data']['datas'], function ($item) {
            return [
        ...
        return $check_data_pengeluaran;
    }

    public function sendPengeluaranData($data)
    {

        $token = $this->getToken();

        $urlApi = 'https://training-bios2.kemenkeu.go.id/api/ws/keuangan/akuntansi/pengeluaran';
